...
The supported types is limited.
You can extend this list registering custom `Converter`:

[source, java]
----
Converter<BigDecimal> customParameterConverter = new Converter<BigDecimal>() {
        @Override
...
parameterConverter.getConverters().add(customParameterConverter);
----
